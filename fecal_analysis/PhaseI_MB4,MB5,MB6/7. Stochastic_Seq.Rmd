...
for (a in taxa_levels){
 ...
 assign(tax_table_name,read.csv(paste(output_location_sto,output_run_list[1],"\\Taxa\\taxa_Summary_",b,"_",run_list[1],".csv",sep=""),row.names=1))
}
...
```{r}
samplelist_seqcontrols<-rownames(physeq_complete1)
observed_baseline_seqcontrols<- data.frame()
taxa_observed=colnames(physeq_complete1)
...
write.csv(observed_baseline_seqcontrols,paste(output_location_sto,"Data\\Baseline\\observed_baseline_",taxa_level,"_seqcontrols.csv",sep=""))
```
...
write.csv(relabun_baseline_seqcontrols,paste(output_location_sto,"Data\\Baseline\\relabun_baseline_seqcontrols.csv",sep=""))
```
...
for (a in output_run_list){
 ...
 for (b in taxa_levels){
  ...
  assign(taxa_table_name,read.csv(paste(output_location_sto,"\\Taxa\\",b,"\\",a,"_observed_baseline_seqcontrols.csv",sep=""),row.names=1))
 }
}
...
write.csv(obs_merged,paste(output_location_sto,"Summary\\obs_diffs.csv",sep=""))
```
