...
write.csv(tar_tdr,paste(output_location,"\\Taxa\\Taxonomy_tar_trd_ext.csv",sep=""))
...
tar_tdr_av_kit<-function(db,opts){
 ...
 if(opts=="STD"){
  ...
  for(a in rownames(temp)){
   temp[a,"Kit"]<-opts
   temp[a,"Control"]<-a
   rownames(temp)[count]<-paste(a,opts,sep="_")
   count=count+1
  }
 }
...
filt_list="STD"
tar_tdr_av<-tar_tdr_av_kit(tar_tdr,filt_list)
write.csv(tar_tdr_av,paste(output_location,"\\Taxa\\Taxonomy_tar_trd_ext_av.csv",sep=""))


#Run by kit
tar_tdr_av<-data.frame("Expected"=1,"TAR"=1,"TDR"=1,"F1Score"=1,"Kit"=1,"Control"=1)
