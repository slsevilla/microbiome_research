...
relabun_av<-function(db,taxadb,exptaxadb,type,subset){
  ...
  for (a in unique(db[,"Source.Descrip"])){
    ...
    for(b in unique(temp[,subset])){
      ...
      if(nrow(temp2)>0){
        rowcount=nrow(relabun_extcont_av)+1
        for (c in taxa_list){
          temp3<-mean(temp2[,c])
          relabun_extcont_av[rowcount,"Name"]<-paste(a,b,sep="_")
          relabun_extcont_av[rowcount,"Subset"]<-b
          relabun_extcont_av[rowcount,"Source.Descrip"]<-a
          relabun_extcont_av[rowcount,c]<-temp3
        } 
      }
    }
    ...
    if(a!="Water"){
      for (b in rownames(exptaxadb)){
        relabun_extcont_av[rowcount,"Name"]<-paste(a,"expected",sep="_")
        relabun_extcont_av[rowcount,b]<-as.numeric(as.character(exptaxadb[b,a]))
      } 
    }
  }
  
  relabun_extcont_av[relabun_extcont_av=="A" ]=0 #replace A's with 0
  relabun_extcont_av[is.na(relabun_extcont_av)]=0 #replace A's with 0
  
  rownames(relabun_extcont_av)<-relabun_extcont_av$Name
  relabun_extcont_av<-relabun_extcont_av[,-1]
  write.csv(relabun_extcont_av,paste(output_location,"Data/relabun_extcont_av_",type,"_",subset,".csv",sep=""))
  return(relabun_extcont_av)
}
 
#Run functions
relabun_extcont<-relabun(taxa_Genus_greengenes,expected_taxa_Genus,metadata_Genus_greengenes)
relabun_extcont_av<-relabun_av(relabun_extcont,taxa_Genus_greengenes,expected_taxa_Genus,"std","Source.Descrip")
...
relabun_av_plot<-function(db,type){
  ...
  relabun_extcont_av_plot<-data.frame()
  ...
  for (a in unique(db$Source.Descrip)[unique(db$Source.Descrip)!="Water"&unique(db$Source.Descrip)!="0"]){
    for (b in rownames(expected_taxa_Genus[expected_taxa_Genus[,a]!='A',])){
      for (c in unique(relabun_extcont$Ext.Kit)){
        rowcount<-nrow(relabun_extcont_av_plot)+1
        relabun_extcont_av_plot[rowcount,"Exp"]<-as.numeric(as.character(expected_taxa_Genus[b,a]))
        relabun_extcont_av_plot[rowcount,"Obs"]<-db[paste(a,c,sep="_"),b]
        relabun_extcont_av_plot[rowcount,"Tax"]<-b
        relabun_extcont_av_plot[rowcount,"Source.Descrip"]<-a
        relabun_extcont_av_plot[rowcount,"Ext.Kit"]<-c
      }
    }
  }
  relabun_extcont_av_plot[is.na(relabun_extcont_av_plot)]=0 #replace A's with 0
  write.csv(relabun_extcont_av_plot,paste(output_location,"Data/relabun_extcont_av_",type,"_plot.csv",sep=""))
}

relabun_av_plot(relabun_extcont_av,"std")


relabun_extcont_avplot_list <- list()
