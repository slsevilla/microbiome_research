 ...
 for (b in samplelist_extcontrols){
  for (c in genus_list){
   ...
   if(control_type!="Water"){
    sample_name<-paste(control_type,"_Expected",sep="")
    kit_name <- a
...
for (a in samplelist_extcontrols){
 for (b in genus_list){
  control_type<-metadata_baseline[a,"Source.Descrip"]
  
  relabun_ob <- relabun_baseline_extcontrols[a,b]
  relabun_ex <- relabun_baseline_extcontrols[control_type,b]
  sample_name  <- a
  genus_temp <- b
  
  if(relabun_ex!=0){ #Only need genus that are expected for comparisons
