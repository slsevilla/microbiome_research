...
for(a in taxa_levels){
 for (b in reference_db){
  ...
  temp <- read.table(paste(output_location,"Taxa\\",metadata_db_name,"_extcontrol.csv",sep=""),sep=",",header= TRUE)
  ...
  temp<-read.table(paste(output_location,"Taxa\\",taxa_db_name,"_extcontrol.csv",sep=""),sep=",",header= TRUE)
  rownames(temp)<-temp$X
  temp<-temp[-1]
