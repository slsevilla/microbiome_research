...
p1<-ggplot(sample_sum_df, aes(x = sum, fill=Ext.Kit)) + 
  ...
  ylab("Number of Samples") +
  ggtitle("Distribution of Sample Sequencing Depth - Post-Filtering")
file_name =paste(project_location,"\\Output\\R\\Graphs\\Summary\\SeqDepth_postfilter_blanks.tiff",sep="")
tiff(file_name, width=800)
print(p1)
dev.off()
...
for (a in phylos_obs){
 ...
 file_name =paste(project_location,"\\Output\\R\\OTUs\\otu_Summary_family",count,".csv",sep="")
 write.csv(famtab,file_name)
 
 metatab <- as.data.frame(sample_data(a))
 file_name =paste(project_location,"\\Output\\R\\OTUs\\metadata_Summary_family",count,".csv",sep="")
 write.csv(metatab,file_name)
 count=count+1
}
