...
output_location=c("T:\\DCEG\\CGF\\TechTransfer\\Microbiome\\Extraction\\Optimization\\Fecal\\Fresh Fecal Optimization_2017.08\\Phase I\\Analysis\\NP0084-MB4,5,6\\R\\")
...
for (a in project_name){
 ...
 #Create phylo object
 paste("physeq_complete",count,sep="")<-phyloseq(otu_table(otus$data, taxa_are_rows = T), phy_tree(tree$data), tax_table(tax_table), sample_data(metadata))
 
 count=count+1
}
```

#Filter and merge phylo objects
```{r}
count=1
#Create filtered list of samples
for (a in project_name){

 paste("samplelist1_passed_count",sep="")<-as.list(sample_names(sample_data(physeq_complete)))
...
#7 Save data tables for downstream use - Genus
```{r}
