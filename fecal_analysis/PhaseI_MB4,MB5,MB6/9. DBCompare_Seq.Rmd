...
for (a in reference_db){
 for (b in taxa_levels){
  ...
  assign(tax_table_name,read.csv(paste(output_location_seq,b,"\\Taxa\\taxa_",b,"_",a,".csv",sep=""),row.names = 1,check.names = FALSE))
 }
}
```
...
for (a in taxa_levels){
 ...
 tiff(paste(output_location_seq,a,"\\Graphs\\venn_",b,".tiff",sep=""),compression="lzw")
 grid.draw(final)
 dev.off()
}
