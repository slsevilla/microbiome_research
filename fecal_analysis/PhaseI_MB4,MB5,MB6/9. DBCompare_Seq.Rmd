...
for (a in reference_db){
 ...
 write.csv(o, file = paste(output_location_seq,"Summary\\otu_table_filt_",a,".csv",sep=""))
}
...
for (a in reference_db){
  ...
  for (c in rownames(taxtab)){
   ...
   if(!a=="greengenes"){
    ...
    if(grepl("Unassigned",taxa)){ #Silva has unassigned tax 
     rownames(taxtab)[rownames(taxtab)==c]<-"Unassigned"
     ...
     if(!grepl("D_5__",taxa)){ #if the name includes a D_ and number less than 5, it will be from a higher tax level
      rownames(taxtab)[rownames(taxtab)==c]<-"HigherGenus"
     } else{
      colname_update<-gsub("D_\\d__","",c) #Need to remove the formatting of taxonmy for each viewing downstream
      rownames(taxtab)[rownames(taxtab)==c]<-colname_update
     }
     #If not Genus
    } else
     if(!grepl("D_4__",taxa)){
      rownames(taxtab)[rownames(taxtab)==c]<-"HigherFamily"
     } else{
      colname_update<-gsub("D_\\d__","",c)
      rownames(taxtab)[rownames(taxtab)==c]<-colname_update
     }
    #If greengenes
    } else{    
 ...
 #  taxtab<-taxtab[-1,]
 #  
 #  file_name =paste(output_location_seq,b,"\\Taxa\\taxa_Summary_",b,"_",a,".csv",sep="")
 #  write.csv(taxtab,file_name)
 #  
 #  metatab <- as.data.frame(sample_data(get(phy_filt_name)))
 #  file_name =paste(output_location_seq,b,"\\Taxa\\metadata_Summary_",b,"_",a,".csv",sep="")
...
for (a in reference_db){
 ...
 assign(otu_table_name,read.csv(paste(output_location_seq,"Summary\\otu_table_filt_",a,".csv",sep="")))
...
otu_diff_119to132 <- compare_df(otu_silva,otu_silva132)
#Result: Tables are the same. No comparison needed at taxonomy levels.

#Compare OTU before any filtering
o<-otu_table(physeq_complete_silva)
o2<-otu_table(physeq_complete_silva)
otu_diff_119to132 <- compare_df(o,o2)
#Result: Tables are the same. No comparison needed at taxonomy levels.

#Taxonomic file
o<-as.data.frame(tax_table(physeq_complete_silva))
o2<-as.data.frame(tax_table(physeq_complete_silva))
o<-o[,1:15]
o2<-o2[,1:15]
otu_diff_119to132 <- compare_df(o,o2)
