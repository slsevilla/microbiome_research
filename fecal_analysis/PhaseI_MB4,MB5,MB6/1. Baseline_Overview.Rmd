...
#4. Filter samples with less than 10000 reads
```{r}
...
p2<-ggplot(sample_sum_df, aes(x = sum, color=Sample.Cat)) + 
  geom_histogram(fill="white",binwidth = 2500) +
  xlab("Read counts") +
  theme(axis.title.y = element_blank())
...
a<-summary(sample_data(physeq_filt)$Source.Descrip)
capture.output(a, file = "Data/Baseline/summary_sourcedesc_postfilter.txt")
```
