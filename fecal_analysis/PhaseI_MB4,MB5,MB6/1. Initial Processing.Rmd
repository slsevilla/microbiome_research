...
controls<-c("Seq.Control")
controls<-c("Ext.Control")
...
sig_mul_values<-function(phyloobject,testname,filtercat){
 ...
 results<-merge(results,sample_data(physeq_scale)$Source.Descrip)
...
#Run test
test_list<-c('Observed','Chao1','ACE')
sub_type<-c("Source.Descrip")

for (a in test_list){
 
 sig_mul_values(physeq_scale,a,sub_type)
}
```
