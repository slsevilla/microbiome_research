...
project_name=c("Project_NP0084_MB4\\DB_Variations")
manifest_name=c("NP0084-NP0084-MB4_08_29_19_metadata_seq.txt")

output_location=c("T:\\DCEG\\CGF\\TechTransfer\\Microbiome\\Extraction\\Optimization\\Fecal\\Fresh Fecal Optimization_2017.08\\Phase I\\Analysis\\NP0084-MB4,5,6\\R_DB_Seq2\\")

sample_depth=100000
reference_db=c("silva")
...
subset_list<-c("Ext.Control") #Ext.Control Seq.Control Study
physeq_scale_sub<-subset_samples(physeq_scale, Sample.Type %in% subset_list)
...
file_name<-c("extcontrol_hmo") #all, seqcontrol, extcontrol_std, extcontrol_hmo, blanks, replicate

#Run protocol
for (a in taxa_levels){
 
 split_phyloobject(a,physeq_scale_sub,reference_db,file_name) #change object 
}
```