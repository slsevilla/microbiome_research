...
project_name=c("Project_NP0084_MB4\\Stochastic_Run1")
manifest_name=c("NP0084-MB4_08_29_19_metadata_seq.txt")

output_location=c("T:\\DCEG\\CGF\\TechTransfer\\Microbiome\\Extraction\\Optimization\\Fecal\\Fresh Fecal Optimization_2017.08\\Phase I\\Analysis\\NP0084-MB4,5,6\\R_Stochastic\\R_Run1\\")
...
file_name =paste(output_location,"Graphs\\SeqDepth_postfilter.tiff",sep="")
tiff(file_name, width=800)
print(p1)
dev.off()
...
p1<-ggplot(sample_sum_df, aes(x = sum, fill=Run.ID)) + 
  ...
  ylab("Number of Samples") +
  ggtitle("Distribution of Sample Sequencing Depth - Post-Filtering")
file_name =paste(output_location,"Graphs\\SeqDepth_postfilter_runid.tiff",sep="")
tiff(file_name, width=800)
print(p1)
dev.off()
...
p1<-ggplot(sample_sum_df, aes(x = sum, fill=Ext.Kit)) + 
  ...
  ylab("Number of Samples") +
  ggtitle("Distribution of Sample Sequencing Depth - Post-Filtering")
file_name =paste(output_location,"Graphs\\SeqDepth_postfilter_blanks.tiff",sep="")
tiff(file_name, width=800)
print(p1)
dev.off()
...
filt_controls<-function(phyobject,filteroption,filterselection){
 temp<-subset_samples(phyobject, filteroption %in% filterselection)
 return(temp)
}
...
#controls<-c("Ext.Control","Ext.Blank")
#controls<-"Replicate"
physeq_control<-subset_samples(physeq_scale, Sample.Type %in% controls)
rare_curves(physeq_control,"Run.ID","Rarecurve_PostFiltering_SeqControls")
```

#Alpha Diversity - All samples, Run ID
```{r}
plot_diversity<-function(phyobject,filter){
 p<-plot_richness(phyobject, x=filter) + geom_boxplot()
 file_name =paste(output_location,"Graphs\\alphadiv_",filter,".tiff",sep="")
 tiff(file_name, width=800)
 print(p)
 dev.off()
} 
...
sig_values<-function(phyloobject,testname,filtercat,filtlist){
 ...
 if(test1$p.value<.05 || test1$p.value<0.5){
  ...
  if(test1$p.value<0.05){
   print ("********* Results: Paring does not pass variance test (p value <0.05; unequal variance will be used in t-test")
   print(var.test(samp1,samp2))
   
   test1<-t.test(samp1, samp2,var.equal = FALSE)
   
  } else{
   print ("********* Results: Paring does pass the variance test (p value >0.05")
   test1<-t.test(samp1, samp2)
  }
  
  #Print result meaning
  if(test1$p.value<0.05){
   cat ("********* Results: The results from the",testname,"are significant")
  } else{
   cat ("********* Results: The results from the",testname,"are NOT significant")
  }
  print(test1)
 }
}
...
#Run test
sig_paring<-c("MSA1000","MSA1001")
test_list<-c('Shannon')

for (a in test_list){
 sig_values(physeq_scale,test_list,"Source.Descrip",sig_paring)
