...
```{r}
samplelist_study <- subset(metadata_baseline,Sample.Cat!="Ext.Blank") #Remove EB's since they have no expected values
samplelist_study <- rownames(metadata_baseline)
observed_count=0
...
samplelist_study_nowater <- subset(metadata_baseline,Source.Descrip!="Water") #only include controls
samplelist_study_nowater <- rownames(samplelist_study_nowater)
relabun_baseline_extcontrols_otoe<-data.frame()
count=1

for (a in samplelist_study_nowater){
 for (b in genus_list){
