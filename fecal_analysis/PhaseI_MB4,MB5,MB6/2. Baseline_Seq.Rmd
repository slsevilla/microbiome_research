...
physeq_seq<-subset_samples(physeq_scale, Sample.Type %in% subset_list)
```
...
```{r}
colnames(tar_tdr)
tar_tdr_av<-aggregate(tar_tdr[, 8], list(tar_tdr$ControlType), mean)
colnames(tar_tdr_av)<-c("Control.Type","F1Score")
