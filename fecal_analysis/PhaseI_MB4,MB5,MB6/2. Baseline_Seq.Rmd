...
manifest_name="NP0084-metadata_Summary_1.csv"
sample_depth = 20000
```
...
otu_baseline_complete <- read.csv("OTUs/otu_baseline_1.csv")
colnames(otu_baseline_complete)[colnames(otu_baseline_complete)=="X"] <- "SampleID"
```

#3. Filtering of samples to include project set only
```{r}
#Remove failed samples from info database
metadata_baseline_filtered <-subset(metadata_baseline_complete, !(SampleID %in% samplelist_baseline_failed$x))
...
```{r}
otu_baseline_filtered <- subset(otu_baseline_complete, !(SampleID %in% samplelist_baseline_failed$x))
row.names(otu_baseline_filtered)<-otu_baseline_filtered$SampleID
otu_baseline_filtered<-otu_baseline_filtered[,-1]
...
relabund_baseline_seqblanks<- data.frame()

for (a in samplelist_baseline_water){
 
 for (i in 1:(ncol(otu_baseline_filtered)-1)){
  relab <- otu_baseline_filtered[a,i]
  genus <- colnames(otu_baseline_filtered[i])
   
  if(!(is.na(relab) | is.null(relab))){
     relabund_baseline_seqblanks[a,genus] <- relab/otu_baseline_filtered[a,"Total_OTUs"]
  }
 }
}

write.csv(relabund_baseline_seqblanks,"Data/Baseline/relabun_seqbaseline_blanks.csv")
```
