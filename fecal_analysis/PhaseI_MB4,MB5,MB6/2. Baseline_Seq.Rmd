...
relabun_sig<-function(phyobj,taxa_level,environments,ctname){
 ...
 # tiff(paste(output_location,"\\Graphs\\","ABGH_",ctname,".tiff",sep=""),compression="lzw")
 # grid.draw(plot(family.AGBH,aes(col=group,shape=type)))
 # dev.off()
 ...
 if(nrow(sub.AGBH>0)){
  ...
  tiff(paste(output_location,"\\Graphs\\","ABGH_sig_",ctname,".tiff",sep=""),compression="lzw")
  grid.draw(p1)
  dev.off()
 }
 
 # taxa_names(temp_phyobj)<-1:length(phy_tree(temp_phyobj)$tip.label) #change OTU names to numbers
 # phy_tree(temp_phyobj)$node.label<-1:length(phy_tree(temp_phyobj)$node.label) #change node labels to numbers
