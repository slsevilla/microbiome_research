...
count=1
for (a in colnames(otu_baseline_filtered)){
 if(grepl("g__",a)){
  colnames(otu_baseline_filtered)[colnames(otu_baseline_filtered)==a] <- str_remove(a, "g__")
 }
}
...
for (a in samplelist_baseline_water){
 ...
 for (b in ncol(otu_baseline_filtered)-1){
   ...
   if(!(is.na(relab) | is.null(relab))){
     relab_baseline_seqblanks[a,b] <- otu_baseline_filtered[a,b]/otu_baseline_filtered[a,"Total_OTUs"]
   }
...
write.csv(relab_baseline_seqblanks,"/datatables/relabun_seqbaseline_blanks.csv")
